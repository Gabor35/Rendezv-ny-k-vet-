...
namespace esemenyrendezo.Controllers
{
    ...
    public class JelszoController : ControllerBase
    {
        ...
        public async Task<IActionResult> JelszoMosositas(string loginName, string oldPassword, string newPassword)
        {
            try
            {
                using (var context = new EsemenyrendezoContext())
                {
                    Felhasznalo? user = context.Felhasznalos.FirstOrDefault(f => f.FelhasznaloNev == loginName);
                    if (user != null)
                    {
                        if (Program.CreateSHA256(oldPassword) == user.Hash)
                        {
                            user.Hash = Program.CreateSHA256(newPassword);
                            context.Felhasznalos.Update(user);
        ...
        public async Task<IActionResult> ElfelejtettJelszo(string Email)
        {
            using (var context = new EsemenyrendezoContext())
            {
                try
                {
                    var user = context.Felhasznalos.FirstOrDefault(f => f.Email == Email);
                    if (user != null)
                    {
                        if (string.IsNullOrEmpty(user.Email))
                        {
                            return BadRequest("A felhasználóhoz nem tartozik érvényes e-mail cím.");
                        }
                        string jelszo = Program.GenerateSalt().Substring(0, 16);
                        user.Hash = Program.CreateSHA256(Program.CreateSHA256(jelszo + user.Salt));
                        context.Felhasznalos.Update(user);
                        await context.SaveChangesAsync();
                        await Program.SendEmail(user.Email, "Elfelejtett jelszó", "Az új jelszava: " + jelszo);
